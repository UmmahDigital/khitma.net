...
export function GET_JUZ_READ_EXTERNAL_URL(juzIndex: number): string {
  const JUZ_START_PAGE = [
    1, 22, 42, 62, 82, 102, 122, 142, 162, 182, 202, 222, 242, 262, 282, 302,
  ...
  let url =
    'https://app.quranflash.com/book/Medina1?ar#/reader/chapter/' +
    (JUZ_START_PAGE[juzIndex] + PAGE_OFFSET);
...
export class KhitmaGroup_Sequential extends KhitmaGroup {
  ...
  public getMyAjzaIndexes(owner) {
    let myAjza = this.ajza.filter(
      (juz) => juz.owner === owner && juz.status === JUZ_STATUS.BOOKED
    );
  ...
  static convertAjzaToArray(ajza: object): Juz[] {
    ...
    return Object.values(ajza).sort((a: any, b: any) =>
      a.index > b.index ? 1 : -1
    );
  }
}
...
export class KhitmaGroup_SameTask extends KhitmaGroup {
  ...
  public getCounts() {
    return {
      ...
      done: this.members.filter(function (item) {
        return item.isTaskDone;
      }).length,
    };
  }
